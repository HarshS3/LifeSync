...
async function analyzeMealTiming(userId, date) {
  ...
  const [workouts, user] = await Promise.all([
    ...
    User.findById(userId).select('biologicalProfile weight remiders.reminderTimes')
  ]);
  ...
  workouts.forEach(workout => {
    ...
    // Check Post-workout (within 120 min)
    ...
    if (postWorkoutMeal) {
      ...
      if (p < 25) {
        alerts.push({
          ...
          text: `Your post-workout meal was light on protein (${Math.round(p)}g). Aim for 25-40g within 2 hours of "${workout.name}" to maximize muscle protein synthesis.`
        });
      }
    } else {
      alerts.push({
        ...
        text: `No significant protein intake detected within 2 hours after your "${workout.name}" workout. Get 25g+ protein now to kickstart recovery.`
      });
    }
  });
  ...
  const lastMeal = meals.reduce((prev, curr) => {
    const prevMin = toMin(prev.time);
    const currMin = toMin(curr.time);
    return (currMin > prevMin) ? curr : prev;
  });

  const lastMealMin = toMin(lastMeal.time);
